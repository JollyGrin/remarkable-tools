...
// Helper function to upload a file buffer to UploadThing
export async function uploadFileBufferToUploadThing(
  fileBuffer: Buffer,
  fileName: string,
) {
  try {
    console.log(`Starting upload of file buffer: ${fileName} (${fileBuffer.length} bytes)`);

    // Check if we have an API key and log it safely (only first few chars)
    const apiKey = process.env.UPLOADTHING_SECRET;
    if (!apiKey) {
      throw new Error("UPLOADTHING_SECRET not set in environment variables");
    }
    ...
    console.log("Uploading file buffer to UploadThing REST API...");
    ...
    // Add the file as the last field - convert buffer to blob
    ...
    console.error("Error uploading file buffer to UploadThing:", error);
    if (error instanceof Error) {
      console.error("Error message:", error.message);
      console.error("Error stack:", error.stack);
    }
